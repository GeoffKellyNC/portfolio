...
import {EMAIL_JS_KEY, SERVICE_ID, TEMPLATE_ID} from '../../../constants'
...
function ContactForm() {
  ...
  const handelName = (evt) => { setSenderName(evt.target.value) }

  const handelEmail = (evt) => { setSenderEmail(evt.target.value) }

  const handelMessage = (evt) => { setMessage(evt.target.value) }
  ...
  className = 'success-message'
                            style={{
    'color': '#F0A202', 'fontSize': '1.5em', 'textAlign': 'center',
        'fontFamily': 'Poppins, sans-serif'}}>
            ...
            <StyledContactForm onSubmit={sendMail} className = 'contact-form'>
                <input type = 'text'
                            ...
                            placeholder = 'Name'
                            onChange = {handelName} className =
                                'input-box' / > < input type = 'email'
                            name = 'senderEmail'
                            value = {senderEmail} required
                            placeholder = 'Email'
                            onChange = {handelEmail} className =
                                'input-box' / > < textarea name = 'message'
                            value = {message} required
                            placeholder = 'Message'
                            onChange = {handelMessage} className = 'input-box'
                            rows = '5'
                            cols =
                                '40' / >
